...
(()=>{
  const cfg={
    cubs:{sport:'baseball/mlb',team:'16',match:['CHC','16'],query:'?season=2026&seasontype=2'},
    wisc:{sport:'volleyball/womens-college-volleyball',team:'275',match:['WIS','275'],query:'?season=2026'},
    nd:{sport:'football/college-football',team:'87',match:['ND','87'],query:'?season=2026'},
    gb:{sport:'football/nfl',team:'9',match:['GB','9'],query:'?season=2026'},
    buf:{sport:'football/nfl',team:'2',match:['BUF','2'],query:'?season=2026'},
    ind:{sport:'football/nfl',team:'11',match:['IND','11'],query:'?season=2026'},
    lfc:{sport:'soccer/eng.1',team:'364',match:['LIV','364'],query:''}
  };
  const cleanRank=s=>String(s||'').replace(/(?:#\d+\s*)+/g,'').trim();
  ...
  async function hydrateTeam(id){
    ...
    try{
      ...
      if(games.length){mergeGames(t,games); if(id==='wisc'&&typeof window.applyScheduleRankings==='function')window.applyScheduleRankings();}
    }catch(_){ }
  }
