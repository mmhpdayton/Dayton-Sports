(()=>{
  ...
  const AVCA_FALLBACK=[
    [1,'Nebraska'],[2,'Pittsburgh'],[3,'Kentucky'],[4,'Stanford'],[5,'Wisconsin'],[6,'Louisville'],[7,'Arizona State'],[8,'SMU'],[9,'Texas'],[10,'Creighton'],[11,'Minnesota'],[12,'Texas A&M'],[13,'Penn State'],[14,'Purdue'],[15,'Southern California'],[16,'Kansas'],[17,'Florida'],[18,'TCU'],[19,'North Carolina'],[20,'Baylor'],[21,'Indiana'],[22,'Tennessee'],[23,'Colorado'],[24,'BYU'],[25,'Oregon']
  ];
  function rowsFromData(kind,fallback){
    const rows=(APP?.data?.rankings?.[kind]||[])
      .filter(r=>Number(r?.rank)>0&&Number(r?.rank)<=25&&r?.team)
      .sort((a,b)=>Number(a.rank)-Number(b.rank));
    return rows.length>=20?rows.slice(0,25).map(r=>[Number(r.rank),r.team]):fallback;
  }
  ...
  function render(){
    const ap=rowsFromData('cfb',AP_FALLBACK);
    const avca=rowsFromData('volleyball',AVCA_FALLBACK);
    const cfb=document.querySelector('#cfbRankingsHub');
    if(cfb)cfb.innerHTML=`<div class="ranking-columns"><section class="ranking-card"><div class="ranking-card-head"><div><div class="kicker">COLLEGE FOOTBALL</div><h3>AP Top 25</h3></div><span class="note">Current data</span></div>${table(ap,'Notre Dame')}<div class="poll-note">Notre Dame highlighted.</div></section><section class="ranking-card"><div class="ranking-card-head"><div><div class="kicker">COLLEGE FOOTBALL</div><h3>Coaches Poll</h3></div><span class="note">Aug. 4, 2026</span></div>${table(COACHES,'Notre Dame')}<div class="poll-note">Notre Dame highlighted.</div></section></div>`;
    const vb=document.querySelector('#vbRankingsHub');
    if(vb)vb.innerHTML=`<section class="ranking-card"><div class="ranking-card-head"><div><div class="kicker">NCAA WOMEN</div><h3>AVCA Top 25</h3></div><span class="note">Current poll</span></div>${table(avca,'Wisconsin')}<div class="poll-note">Wisconsin highlighted.</div></section>`;
  }
  ...
  window.addEventListener('load',()=>{
    ...
    document.querySelectorAll('.hub-tab').forEach(b=>b.addEventListener('click',()=>{showPane(b.dataset.hub);render();}));
    showPane('standings');
  });
})();
